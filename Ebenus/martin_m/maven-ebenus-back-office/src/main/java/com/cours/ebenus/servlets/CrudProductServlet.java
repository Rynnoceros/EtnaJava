...
public class CrudProductServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        if (UserConnectedManager.getInstance().isAdministrateur()) {
            allProducts = serviceFacade.getProduitDao().findAllProduits();
            request.setAttribute("allProducts", this.allProducts);
        ...
        if (UserConnectedManager.getInstance().isAdministrateur()) {
            if (param != null) {
                ...
                if (id > 0) {
                    ...
                    toDelete.setIdProduit(id);
                    boolean deleted = serviceFacade.getProduitDao().deleteProduit(toDelete);
                    if (deleted) {
                        allProducts.removeIf(filterById(id));
    ...
    private Predicate<Produit> filterById(int id) {
        return p -> p.getIdProduit() == id;
    }
